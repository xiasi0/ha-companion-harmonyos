...
The current app version is `1.3.0` (`versionCode`: `1030000`) and the declared target device types are `phone` and `tablet`. The project is still under active development: several core paths are runnable, while some platform capabilities still need more real-device validation and production hardening.
